...
#include "../Dialog/MyExcelReadDlg.h"
#include <math.h>
...
//add by lc
void CStockGraph::OnReadExcelData( )
{
	//k line for day
	m_nCurKType = CKData::ktypeDay;
	CKData	&kdata	=	m_CurStock.GetKData( m_nCurKType );
	...
	kd.m_date	=	CSPTime::GetStockTimeNext( kd.m_date, m_nCurKType );
*/
	CMyExcelReadDlg	dlg( m_pParent );
	if( IDOK == dlg.DoModal( ) )
	{
	    //get all data from excel, and put into kd.
		pKd	=	dlg.GetPKDATA( );	
		dataSize = dlg.GetDataSize();
		
		for (i = 0;i < dataSize;i++)
		{
			kdata.Add( pKd[i] );		
		    m_CurStockDown.GetKData( m_nCurKType ).Add( pKd[i] );
		}
